...
def my_decode_ffn16(dev, embed_dim=2048, hidden_dim=8192, group_size=32,
    ...
    gate_up = Kernel("layer_fused_gate_up_bcast_bf16", "layer_fused_relay.o",
    ...
    def ffn_body(wf, bc, pp, gate_up_fn, silu_fn, down_fn):
        for _ in range_(0xFFFFFFFF):
            b = bc.acquire(1)
            for j in range_(gu_tiles):                 # gate → lf_left (K-stream bcast)
                w = wf.acquire(1)
                gate_up_fn(index.casts(T.i32(), j), 0, w, b, 0)
                wf.release(1)
            for j in range_(gu_tiles):                 # up → lf_right (K-stream bcast)
                w = wf.acquire(1)
                gate_up_fn(index.casts(T.i32(), j), 0, w, b, 1)
                wf.release(1)
            bc.release(1)
            silu_fn(Hc16)
            o = pp.acquire(1)
