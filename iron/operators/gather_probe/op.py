...
"""Op wrapper for the cross-column gather probe (see design.py).
Compiles layer_fused.cc for the add (copy) kernel; the gather itself is an
ObjectFifo.join at a MemTile (no kernel)."""
import numpy as np
...
class AIEGatherProbe(AIEOperatorBase):
    ...
    def get_artifacts(self, prefix="gather_probe_"):
        operator_dir = Path(__file__).parent
        E = self.embed_dim
        base = f"{prefix}e{E}_c{self.num_cols}"
        mlir_artifact = PythonGeneratedMLIRArtifact.new(
            f"{base}.mlir",
            import_path=operator_dir / "design.py",
            callback_fn="my_gather",
            callback_args=[self.context.device_manager.device_type,
                           E, self.num_cols],
        )
        kobj = KernelObjectArtifact.new(
            "layer_fused_gather.o",
    ...
    def set_up_runtime(self):
        E = self.embed_dim
        slice_n = E // self.num_cols
        self.add_buffer("out", E, dtype=bfloat16)
        self.add_buffer("inp", E, dtype=bfloat16)
        self.add_buffer("zero", slice_n, dtype=bfloat16)
        self.add_kernel("gather", self.xclbin_artifact,
                        self.xclbin_artifact.kernel_name, self.insts_artifact)
        self.add_to_runlist("gather", "out", "inp", "zero")
