...
def my_decode_attn_oproj(dev, embed_dim=2048, K_gemv=2048, head_dim=64, group_size=32,
    ...
    # concat2 reassembles the two attn join halves; copy_k is a plain bf16 copy. Both
    # share the L1_B_ty binding (xb_elems >= E) used by the relay/mux/cycle tiles.
    concat2 = Kernel("attn_concat2_bf16", "attn_concat.o",
                     [L1_HALF_ty, L1_HALF_ty, L1_B_ty, np.int32])
    copy_k = Kernel("attn_copy_bf16", "attn_concat.o", [L1_B_ty, L1_B_ty, np.int32])
    ...
    # attn_out relay: a center tile concats Half0|Half1 -> AttnE (one fifo). AttnE,
    # MuxB and O_out are all typed L1_B_ty (xb_elems >= E) so the copy/concat kernels
    # share ONE bf16 type binding; only the first E elements are meaningful.
    AttnE = ObjectFifo(L1_B_ty, name="AttnE", depth=2)
    O_out = ObjectFifo(L1_B_ty, name="O_out", depth=2)
    # CYCLE DE-RISK: head-0's center tile produces Qi[0] (phase1) THEN consumes the
    # attn_out (phase2) — the center->edge->center cyclic dependency the phase-blind
    # layer needs. To fit the 2-S2MM cap, head-0's activation rides a MUX fifo that
    # carries x (phase1) then AttnE (phase2): a mux relay produces both into MuxB, the
    # head-0 worker acquires it twice. Phase2 is a dummy copy AttnE[0:E] -> O_out (no
    # O-weights), isolating the cyclic-dependency question from the O-proj GEMV.
    Bx = ObjectFifo(L1_B_ty, name="Bx", depth=1)            # head-0 dedicated x source
    MuxB = ObjectFifo(L1_B_ty, name="MuxB", depth=2)         # [x | AttnE-padded]
    ...
    for h in range(NH):
        ...
        if h == 0:
            # CYCLE de-risk: head-0 does Q-GEMV (phase1, MuxB=x) -> Qi[0], then
            # consumes MuxB=AttnE (phase2) and copies it to O_out. Same tile produces
            # Qi[0] then consumes attn_out (which depends on Qi[0] via the edges).
            def cyc_body(af, mb, qf, oo, gemv_fn, rope_fn, copy_fn):
                for _ in range_(0xFFFFFFFF):
                    b = mb.acquire(1)                 # phase1: x bundle
                    q = qf.acquire(1)
                    for j in range_(gemv_tiles):
                        a = af.acquire(1)
                    ...
                    qf.release(1)
                    mb.release(1)
                    b2 = mb.acquire(1)               # phase2: AttnE (cyclic input)
                    o = oo.acquire(1)
                    copy_fn(b2, o, embed_dim)
                    mb.release(1); oo.release(1)
            workers.append(Worker(
                cyc_body, [A_f[0].cons(), MuxB.cons(), Qi[0].prod(), O_out.prod(),
                           gemv, rope, copy_k],
    ...
    # mux relay: produce x (phase1) then AttnE (phase2) into MuxB; head-0 acquires it
    # twice. This is what couples the cyclic dependency onto head-0's single activation
    # input within the 2-S2MM cap.
    def mux_body(bx, ae, mb, copy_fn):
        for _ in range_(0xFFFFFFFF):
            x = bx.acquire(1); m1 = mb.acquire(1)
            copy_fn(x, m1, xb_elems)
            bx.release(1); mb.release(1)
            a = ae.acquire(1); m2 = mb.acquire(1)
            copy_fn(a, m2, embed_dim)
            ae.release(1); mb.release(1)
    workers.append(Worker(
        mux_body, [Bx.cons(), AttnE.cons(), MuxB.prod(), copy_k],
    ...
    with rt.sequence(L3_O_ty, L3_W_ty, L3_X_ty, L3_K_ty, L3_V_ty) as (o, w, x, k, v):
        ...
        # head-0's dedicated x source for the mux (rides a free edge shim col 6).
        rt.fill(Bx.prod(), x, x_tap, task_group=tg1, placement=shim(6))
