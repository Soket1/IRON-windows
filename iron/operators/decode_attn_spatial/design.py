...
def my_decode_attn_spatial(dev, embed_dim=2048, K_gemv=2048, head_dim=64, group_size=32,
    ...
    def half_tap(half):
        return TensorAccessPattern(
            tensor_dims=(NH * q_rows,), offset=half * (NH // 2) * q_rows,
            sizes=[1, 1, 1, (NH // 2) * q_rows], strides=[0, 0, 0, 1])

    # EXPLICIT shim placement per fill/drain (NOT AnyShimTile): the SequentialPlacer
    # only auto-searches free columns for Any* endpoints, and an all-column compute
    # layout leaves none. Pinning each transfer to its natural column's shim (row 0)
    # skips the auto-search entirely. Center GEMV inputs (A,B) ride the center col
    # shim; edge attn inputs (K,V) ride the edge col shim; the two output halves drain
    # on the join MemTile columns. Each shim carries <= 2 transfers/direction.
    def shim(col):
    ...
    with rt.sequence(L3_O_ty, L3_W_ty, L3_X_ty, L3_K_ty, L3_V_ty) as (o, w, x, k, v):
        ...
        tg3 = rt.task_group()
        rt.drain(Half0.cons(), o, half_tap(0), task_group=tg3, wait=False,
                 placement=shim(center_col_offset))
        rt.drain(Half1.cons(), o, half_tap(1), task_group=tg3, wait=True,
                 placement=shim(center_col_offset + 3))
