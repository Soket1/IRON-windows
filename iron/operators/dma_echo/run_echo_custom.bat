...
REM
REM Usage: run_echo_custom.bat [1|2|3|4|5|6|all]
...
%PYTHON% -c "import sys; sys.path.insert(0, r'%IRON_DIR%'); from iron.operators.dma_echo.design import echo_v1, echo_v2, echo_v3, echo_v4, echo_v5, echo_v6; v=%VERSION%; m = echo_v1('npu2', 256) if v==1 else (echo_v2('npu2', 128) if v==2 else (echo_v3('npu2', 128) if v==3 else (echo_v4('npu2') if v==4 else (echo_v5('npu2') if v==5 else echo_v6('npu2'))))); open(r'%BUILD_DIR%\echo_v%VERSION%.mlir','w').write(str(m)); print('MLIR written')"
if errorlevel 1 (
