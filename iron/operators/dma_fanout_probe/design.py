...
def my_dma_fanout_probe(dev, cols=(2, 3, 4, 5), embed_dim=2048, group_size=32,
    ...
    # fixed total = 1536*4*PACKED = 28.31MB regardless of NCOL
    assert 1536 % NCOL == 0, "NCOL must divide 1536 for a fixed 28.3MB total"
    WT_PER_TILE = 1536 // NCOL                 # rounds per column
    Ntok = 32
    ...
    wcol = WT_PER_TILE * R * PACKED            # per-column bytes
    L3_W_ty = np.ndarray[(NCOL * wcol,), u8]
    L3_O_ty = np.ndarray[(NCOL * R * Ntok,), bf]

    # per-column weight ingress: DDR -> MemTile(col,1) split to R per-tile fifos
    ...
    # NCOL fills + NCOL drains = 12 channels @ N=6 (FFLM-faithful), not 4/col.
    ColTok, CT_parts = [], []
    for i in range(NCOL):
    ...
    def w_tap(i):
        return TensorAccessPattern(tensor_dims=(1, NCOL * wcol), offset=i * wcol,
