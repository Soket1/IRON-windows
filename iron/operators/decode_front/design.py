...
"""decode_front — FFLM-style multi-column gemv→attention chain.

`num_cols` central columns (cols col_offset .. col_offset+num_cols-1), each a
3-tile vertical pipeline processing ONE KV head group:

  tile (c, 2): GEMV + RoPE → Q_int[c] (on-chip ObjectFifo)
  tile (c, 3): score_init + score_rope_q + score_chunk → I_int[c] (on-chip)
  tile (c, 4): value_init + value_accum + value_normalize → O[c] (DDR drain)

Each tile's static buffers (rotated_q, running max/sum) are per-tile.
score_rope_q runs on the score tile (writes that tile's static rotated_q).

DMA budget per shim column: A-weights + B-vector (gemv phase) then K + V
(attention phase), time-multiplexed via task_group barriers → ≤2 S2MM per
column at any time. B (input activation) is broadcast: each column gets the
same input vector. A/K/V are per-column (each column's own KV head group).
RoPE LUT is a passive Buffer (zero runtime DMA).
"""
import numpy as np
...
def my_decode_front(dev, embed_dim=2048, K_gemv=2048, head_dim=64, group_size=32,
    ...
    with rt.sequence(L3_W_ty, L3_X_ty, L3_K_ty, L3_V_ty, L3_O_ty) as (w, x, k, v, o):
        ...
        # phase 3: drain outputs (1 MM2S per col)
        tg3 = rt.task_group()
        for c in range(num_cols):
