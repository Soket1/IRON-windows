...
"""Op wrapper for decode_fuse_col (#11.1 probe1 — 1-column hub plumbing)."""
import numpy as np
...
class AIEDecodeFuseCol(AIEOperatorBase):
    def __init__(self, n=256, context=None):
        self.n = n
        self.xclbin_artifact = None
        self.insts_artifact = None
        AIEOperatorBase.__init__(self, context=context)

    def get_artifacts(self, prefix="decode_fuse_col_"):
        operator_dir = Path(__file__).parent
        base = f"{prefix}n{self.n}"
        mlir_artifact = PythonGeneratedMLIRArtifact.new(
            f"{base}.mlir",
            import_path=operator_dir / "design.py",
            callback_fn="my_decode_fuse_col",
    ...
    def set_up_runtime(self):
        ...
        self.add_kernel("decode_fuse_col", self.xclbin_artifact,
                        self.xclbin_artifact.kernel_name, self.insts_artifact)
        self.add_to_runlist("decode_fuse_col", "O", "X")
