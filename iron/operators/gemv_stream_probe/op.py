...
"""Op wrapper for gemv_stream_probe (#30 (B) realize): weights STREAM in as blocks
(real weight-DMA), X resident. `sym` selects bcast (column-major) or dot (row-major)
per-block kernel — apples-to-apples under streaming."""
import numpy as np
...
class AIEGemvStreamProbe(AIEOperatorBase):
    ...
    @property
    def sym(self):
        return {"bcast": "layer_fused_gemv_bcast_blk_bf16",
                "dot": "layer_fused_gemv_dot_blk_bf16"}[self.kernel]

    def get_artifacts(self, prefix="gemv_stream_"):
        operator_dir = Path(__file__).parent
        base = f"{prefix}{self.kernel}_m{self.M_OUT}_k{self.K}"
        mlir_artifact = PythonGeneratedMLIRArtifact.new(
            f"{base}.mlir",
            import_path=operator_dir / "design.py",
            callback_fn="my_gemv_stream_probe",
