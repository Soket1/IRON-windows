...
// BROADCAST / outer-product GEMV PROBE (#30). the reference design structure: activation x held
...
// the reference design's vconv 0.55 / vunpack 0.48. G must be even (it is: GROUP_SIZE=32).
template <uint32_t N, uint32_t G, uint32_t K>
...
// → the ~11% deficit. the reference design keeps ALL stage intermediates register/static-local
// in ONE monolithic .o; this mirrors that.
...
// Down PARTIAL GEMV (decomp-B, on-chip reference-style): INT4 with K =
...
// plus a residual into one n-elem output: out = P0+P1+P2+P3+resid. reference-style
...
void layer_fused_rms_norm2_bf16(
    ...
    float sum_sq = ::aie::reduce_add(acc);
    float y = sum_sq / n + eps;
    float x2 = 0.5f * y;
    union { float f; uint32_t i; } conv;
    conv.f = y;
    conv.i = 0x5f3759df - (conv.i >> 1);
    float inv_rms = conv.f * (1.5f - x2 * conv.f * conv.f);
        ...
        for (uint32_t g = 0; g < groups_per_row; g += 2)
            {
                ...
                aie::vector<int4, 2 * block_size> I01 = aie::load_v<2 * block_size>(w_row);
                w_row += block_size;
                auto i8_01 = aie::unpack(I01);
                aie::vector<bfloat16, 2 * block_size> wbf2 =
                    aie::to_float<bfloat16>(i8_01, 0);
                aie::vector<bfloat16, block_size> w0 =
                    aie::mul(wbf2.template extract<block_size>(0), sf0_bc).template to_vector<bfloat16>();
                aie::vector<bfloat16, block_size> w1 =
                    aie::mul(wbf2.template extract<block_size>(1), sf1_bc).template to_vector<bfloat16>();
...
static void _down_bcast_chunk(uint32_t j, const uint8_t *__restrict a,
                              bfloat16 *__restrict c_out) {
    constexpr uint32_t KC = INTER_DIM_PER_COL / NCHUNK;
    ::aie::set_rounding(aie::rounding_mode::conv_even);
    const uint32_t chunk = j % NCHUNK;
    const uint32_t block = j / NCHUNK;
    const bfloat16 *xc = lf_silu_buf + chunk * KC;
    const bfloat16 *scales = reinterpret_cast<const bfloat16 *>(a + KC * N / 2);
    aie::accum<accfloat, N> acc = aie::zeros<accfloat, N>();
    for (uint32_t kg = 0; kg < KC / G; kg++) {
        aie::vector<bfloat16, G> xv = aie::load_v<G>(xc + kg * G);
                ...
                AIE_PREPARE_FOR_PIPELINING
                {
                    ...
                    for (uint32_t blk = 0; blk < blocks_per_group; blk++) {
                        ...
                        aie::vector<bfloat16, block_size> as_bf16 =
                            aie::to_float<bfloat16>(as_int8, 0);
    ...
    for (uint32_t gg = 0; gg < gpr; gg++)
        AIE_PREPARE_FOR_PIPELINING {
            ...
            for (uint32_t r = 0; r < m; r++) {
                ...
                ::aie::vector<bfloat16, BS> abf = ::aie::to_float<bfloat16>(a8, 0);
...
// in `K_CHUNK = 32` token chunks (matches the reference design's ct_chunk_size from
// mha.dll RE'd disasm).
//
...
// instructions in the PDI — driving xclbin size up toward the reference design's
